...
| **Weekly** | Fresh dashboard link for your boss (7-day expiry) | `.\.venv\Scripts\python.exe storage.py url` |
...
### 2. Weekly — refresh the dashboard link for your boss
The link expires after 7 days. Generate a new one and send it:
```powershell
.\.venv\Scripts\python.exe storage.py url
```
It renders in the browser (no download) and always shows the latest daily build. *(Once the
permanent CloudFront URL is set up with the cloud team, this weekly step goes away.)*
...
| Fresh dashboard link | `.\.venv\Scripts\python.exe storage.py url` |

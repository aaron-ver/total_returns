...
| 22 | **Hourly CME futures bars** TU/TY/US/WN (=ZT/ZN/ZB/UB) — eventflow study | **SCRAPER** | `eventflow/pull_bars.py` — Yahoo finance chart API, ~2.4y of 1h bars, accretes daily in the pipeline pull. NO open interest (later: small BBG daily `FUT_AGGTE_OPEN_INT`) | `eventflow/cache/bars_*.parquet` | ❌ (public) |
| 24 | **Article-level headline timestamps** (NYT + Guardian) — ⛔ **DISABLED pending license** | SCRAPER (gated) | `eventflow/pull_news_sources.py` — built and validated, but **both providers' free API tiers are non-commercial only** (Guardian free tier also forbids >24h content retention); institutional use requires a commercial arrangement (Guardian commercial tier; NYT via nytlicensing.com). Code is license-gated: no-ops until `NEWS_API_COMMERCIAL_OK=1` + licensed keys. **GDELT (row 23) is the news backbone — its terms explicitly permit unrestricted commercial use.** | `eventflow/cache/articles_*.parquet` | ❌ |
| 23 | **Headline-intensity series** (iran / trump queries, 15-min resolution) | **SCRAPER** | `eventflow/pull_news.py` — GDELT DOC 2.0 API (free, no auth; RATE-LIMITED per IP and the whole firm shares one egress IP → 10s spacing, 2-month chunks, retries daily in the pipeline; a 429 burst = ~1h cooldown, just rerun). Escalation path if API becomes unusable: GDELT's raw 15-min CSV dumps at data.gdeltproject.org (unthrottled file server, heavier parse) | `eventflow/cache/news_*.parquet` | ❌ (public) |
